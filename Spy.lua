...
DEFAULT_CHAT_FRAME.AddMessage = function(frame, text, ...)
    if text == nil then
        geterrorhandler()("Spy tried to send NIL to chat!")
        text = "[Spy nil message prevented]"
...
Spy.db = Spy.db or { profile = {} }
Spy.Version = "1.2"
Spy.DatabaseVersion = "1.1"
Spy.Signature = "[Spy]"
Spy.ButtonLimit = 10
Spy.MaximumPlayerLevel = MAX_PLAYER_LEVEL_TABLE and MAX_PLAYER_LEVEL_TABLE[GetAccountExpansionLevel()] or 60
Spy.MapNoteLimit = 20
Spy.MapProximityThreshold = 0.02
Spy.CurrentMapNote = 1
Spy.ZoneID = Spy.ZoneID or {}
Spy.KOSGuild = {}
Spy.CurrentList = {}
Spy.NearbyList = {}
Spy.LastHourList = {}
Spy.ActiveList = {}
Spy.InactiveList = {}
Spy.PlayerCommList = Spy.PlayerCommList or {}
...
SpyDB.version = SpyDB.version or Spy.DatabaseVersion

SpyPerCharDB = SpyPerCharDB or {}
...
function Spy:CheckDatabase()
	...
	if not Spy.RealmName or not Spy.FactionName or not Spy.CharacterName then
        return
    end
	...
	SpyDB.removeKOSData[Spy.RealmName][Spy.FactionName] = SpyDB.removeKOSData[Spy.RealmName][Spy.FactionName] or {} 	
	SpyDB.removeKOSData[Spy.RealmName][Spy.FactionName][Spy.CharacterName] = SpyDB.removeKOSData[Spy.RealmName][Spy.FactionName][Spy.CharacterName] or {}
...
function Spy:OnDisable()
	...
	DEFAULT_CHAT_FRAME:AddMessage(L["SpyDisabled"] or "Spy disabled")
end
...
function Spy:EnableSpy(value, changeDisplay, hideEnabledMessage)
	...
	if value then
		...
		if not hideEnabledMessage then
            DEFAULT_CHAT_FRAME:AddMessage(L["SpyEnabled"] or "Spy enabled")
        end
	else
...
function Spy:BuildZoneIDTable(zoneIndex, continentName, next, ...)
	zoneIndex = zoneIndex + 1
	local zones = { GetMapZones(zoneIndex) }
...
function Spy:OnInitialize()
	...
	if not Spy.db.profile then Spy.db.profile = {} end
    if not Spy.db.profile.MapDisplayLimit then Spy.db.profile.MapDisplayLimit = "None" end
	self:SetupOptions()
	...
	DEFAULT_CHAT_FRAME:AddMessage(L["LoadDescription"] or "Spy loaded successfully.")

	Spy.MapNoteList = {}
		for i = 1, Spy.MapNoteLimit do
    	Spy.MapNoteList[i] = {
        displayed = false,
        worldIcon = CreateFrame("Frame", "SpyWorldIcon"..i, WorldMapDetailFrame),
        miniIcon = CreateFrame("Frame", "SpyMiniIcon"..i, Minimap),
        continentIndex = nil,
        zoneIndex = nil,
        mapX = 0,
        mapY = 0,
    	...
    	Spy.MapNoteList[i].miniIcon:Hide()
end

if not Spy.CurrentMapNote or Spy.CurrentMapNote < 1 or Spy.CurrentMapNote > Spy.MapNoteLimit then
    Spy.CurrentMapNote = 1
end
...
function Spy:CombatLogEvent(_, timestamp, event, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
	...
	if bit.band(srcFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE 
		and srcGUID and srcName and not SpyPerCharDB.IgnoreData[srcName] then
		...
		if srcType == 0 or srcType == 8 then
			...
			if not playerData or playerData.isGuess then
				learnt, playerData = Spy:ParseUnitAbility(true, event, srcName, srcFlags, select(9, ...))
			end
			...
			if detected then
				...
				if event == "SPELL_AURA_APPLIED" and (select(10, ...) == L["Stealth"] or select(10, ...) == L["Prowl"]) then
	...
	if bit.band(dstFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE 
		and dstGUID and dstName and not SpyPerCharDB.IgnoreData[dstName] then
		...
		if dstType == 0 or dstType == 8 then
			...
			if not playerData or playerData.isGuess then
				learnt, playerData = Spy:ParseUnitAbility(false, event, dstName, dstFlags, select(9, ...))
			end
	...
	if event == "PARTY_KILL" and srcGUID == UnitGUID("player") and dstName then
		local playerData = SpyPerCharDB.PlayerData[dstName]
		if playerData then
...
function Spy:WorldMapUpdateEvent()
	for i = 1, Spy.MapNoteLimit do
		...
		if note.displayed then
			if note.continentIndex and note.zoneIndex and note.mapX and note.mapY then
				Astrolabe:PlaceIconOnWorldMap(WorldMapDetailFrame, note.worldIcon, note.continentIndex, note.zoneIndex, note.mapX, note.mapY)
				Astrolabe:PlaceIconOnMinimap(note.miniIcon, note.continentIndex, note.zoneIndex, note.mapX, note.mapY)
			else DEFAULT_CHAT_FRAME:AddMessage("Spy skipped note with missing coords")
				end
			end
		end
    end
...
function Spy:CommReceived(prefix, message, distribution, source)
    ...
    if not currentZone or not Spy:GetZoneID(currentZone) then return end
    ...
    if Spy.EnabledInZone and Spy.db.profile.UseData then
        if prefix == Spy.Signature and message and source ~= Spy.CharacterName then
            ...
            if not Spy.InInstance or zone == currentZone then
                if not Spy.PlayerCommList[player] then
                    if version and tonumber(version) and tonumber(version) > tonumber(Spy.Version) and not Spy.UpgradeMessageSent then
                        DEFAULT_CHAT_FRAME:AddMessage(L["UpgradeAvailable"] or "A new version of Spy is available.")
                        Spy.UpgradeMessageSent = true
                    end

                    if class and class ~= "" and not Spy.ValidClasses[class] then return end
                    if level and level ~= "" then
                        level = tonumber(level)
                        if not level or level < 1 or level > Spy.MaximumPlayerLevel or math.floor(level) ~= level then
                            return
                        end
                    else
                        level = nil
                    ...
                    if race and race ~= "" then
                        ...
                        if (Spy.EnemyFactionName == "Alliance" and not (race == "Draenei" or race == "Dwarf" or race == "Gnome" or race == "Human" or race == "Night Elf"))
                        or (Spy.EnemyFactionName == "Horde" and not (race == "Blood Elf" or race == "Orc" or race == "Tauren" or race == "Troll" or race == "Undead")) then
                            return
                        end
                    else
                        race = nil
                    end
                    if zone and zone ~= "" and not Spy:GetZoneID(zone) then return end
                    if subZone == "" then subZone = nil end

                    if mapX and mapX ~= "" then
                        mapX = tonumber(mapX)
                        if not mapX or mapX < 0 or mapX > 1 then return end
                        mapX = math.floor(mapX * 100) / 100
                    else
                        mapX = nil
                    end
                    if mapY and mapY ~= "" then
                        mapY = tonumber(mapY)
                        if not mapY or mapY < 0 or mapY > 1 then return end
		    ...
		    if not Spy.PlayerCommList then Spy.PlayerCommList = {} end	
...
function Spy:ShowMapNote(player)
    ...
    local continentIndex, zoneIndex = nil, nil
    if playerData.zone and playerData.zone ~= "" then
        continentIndex, zoneIndex = Spy:GetZoneID(playerData.zone)
    end

    local mapX, mapY = playerData.mapX, playerData.mapY
    if not (continentIndex and zoneIndex and type(mapX) == "number" and type(mapY) == "number") then
        return
    end
    if not (Spy.db.profile.MapDisplayLimit == "None"
        or (Spy.db.profile.MapDisplayLimit == "SameZone" and zoneIndex == currentZoneIndex)
        or (Spy.db.profile.MapDisplayLimit == "SameContinent" and continentIndex == currentContinentIndex)) then
        return
    end

    Spy.MapNoteList = Spy.MapNoteList or {}
    Spy.CurrentMapNote = Spy.CurrentMapNote or 1
    if not Spy.MapNoteList[Spy.CurrentMapNote] then
        Spy.MapNoteList[Spy.CurrentMapNote] = {
            displayed = false,
            worldIcon = CreateFrame("Frame", "SpyWorldIcon"..Spy.CurrentMapNote, WorldMapDetailFrame),
            miniIcon  = CreateFrame("Frame", "SpyMiniIcon"..Spy.CurrentMapNote, Minimap),
            continentIndex = nil, zoneIndex = nil, mapX = 0, mapY = 0,
        }
        Spy.MapNoteList[Spy.CurrentMapNote].worldIcon:Hide()
        Spy.MapNoteList[Spy.CurrentMapNote].miniIcon:Hide()
    end
    ...
    if continentIndex and zoneIndex and mapX and mapY then
        ...
        DEFAULT_CHAT_FRAME:AddMessage("Spy skipped note with missing coords")
    end

    for i = 1, Spy.MapNoteLimit do
        local other = Spy.MapNoteList and Spy.MapNoteList[i]
        if i ~= Spy.CurrentMapNote and other and other.displayed then
            if continentIndex == other.continentIndex and zoneIndex == other.zoneIndex
               and abs(mapX - other.mapX) < Spy.MapProximityThreshold
               and abs(mapY - other.mapY) < Spy.MapProximityThreshold then
                other.displayed = false
                other.worldIcon:Hide()
                Astrolabe:RemoveIconFromMinimap(other.miniIcon)
                for p in pairs(Spy.PlayerCommList or {}) do
